...
def test_parse_all_folder():
    ...
    for q in questions:
        ...
        assert len(q.get_languages()) == 2, "There must be 2 language files in the test_data/all folder"
        assert len(q.get_right_answers()) == 1
        assert len(q.get_wrong_answers()) == 2
        assert len(q.get_right_answers()) == 1
        assert len(q.get_wrong_answers()) == 2
        # Check appendix association for questions with appendix reference
        ...
        if "[este anexo]" in statement_gl:
            ...
            assert appendix.get_title() is not None
            assert appendix.get_url() is not None
